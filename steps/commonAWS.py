...
def get_today_subsummary_file_name(context):
    # today = date.today()
    # Check the S3 bucket has billing files from the current day
    s3 = boto3.resource('s3')
    bucket = s3.Bucket(context.env_vars['billingBucketName'])
    subsummary_today_file_found = False
    ...
    print("billed/subsummary/{}-{}/{}{}{}".format(year, month, context.env_vars['prefixBillingSubSummaryFileName'], year, last_month.strftime("%m")))
    for obj in bucket.objects.all():
        if ("billed/subsummary/{}-{}/{}{}{}".format(year, month, context.env_vars['prefixBillingSubSummaryFileName'], year, month_1)) in obj.key:
            subsummary_today_file_found = True
            return obj.key

    if not subsummary_today_file_found:
        context.testcase.assertTrue(subsummary_today_file_found, msg='Billing files from today NOT FOUND')

def get_today_subdetail_file_name(context):
    # today = date.today()
    # Check the S3 bucket has billing files from the current day
    s3 = boto3.resource('s3')
    bucket = s3.Bucket(context.env_vars['billingBucketName'])
    subdetail_today_file_found = False
    ...
    print("billed/subdetail/{}-{}/{}{}{}".format(year, month, context.env_vars['prefixBillingSubDetailFileName'], year, last_month.strftime("%m")))
    for obj in bucket.objects.all():
        if ("billed/subdetail/{}-{}/{}{}{}".format(year, month, context.env_vars['prefixBillingSubDetailFileName'], year, month_1)) in obj.key:
            subdetail_today_file_found = True
            return obj.key

    if not subdetail_today_file_found:
        context.testcase.assertTrue(subdetail_today_file_found, msg='Billing files from today NOT FOUND')


def get_today_summary_file_name_empty(context):
    # today = date.today()
    # Check the S3 bucket has billing files from the current day
    s3 = boto3.resource('s3')
    bucket = s3.Bucket(context.env_vars['billingBucketName'])
    ...
    context.testcase.assertFalse(summary_today_file_found, msg='Billing files from today were generated')
...
def get_today_detail_file_name_empty(context):
    # today = date.today()
    # Check the S3 bucket has billing files from the current day
    s3 = boto3.resource('s3')
    bucket = s3.Bucket(context.env_vars['billingBucketName'])
    ...
    context.testcase.assertFalse(detail_today_file_found, msg='Billing files from today were generated')
